...
month_summary_sd %>%
  select(Month = month_name, Functional_Group = functionalGroup,
         Mean_dryMass_per_day = mean_dryMass_per_day,
         SD_dryMass_per_day = sd_dryMass_per_day,
         n) %>%
  kable(caption = "Mean and Standard Deviation of Daily Litterfall per Month and Functional Group",
        digits = 3)
```
...
ggplot(month_summary_sd, aes(x = month_name, y = sd_dryMass_per_day, color = functionalGroup, group = functionalGroup)) +
  geom_point(size = 2) +
...
ggplot(month_summary_sd_no_outlier, aes(x = month_name, y = sd_dryMass_per_day, color = functionalGroup, group = functionalGroup)) +
  geom_point(size = 2) +
  scale_color_brewer(palette = "Dark2") +
